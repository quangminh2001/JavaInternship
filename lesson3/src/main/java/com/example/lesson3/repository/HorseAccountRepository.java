...
public interface HorseAccountRepository extends JpaRepository<HorseAccount,Long> {
    ...
    @Query("select h from HorseEntity h join HorseAccount ha on h.id = ha.horse.id join  TrainerEntity t on ha.account1.id = t.id where t.id = ?1 and year(h.foaled) = ?2")
